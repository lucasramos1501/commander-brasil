...
export default {
    async login(request, response) {
        ...
        if (!user) return response.status(401).send({ auth: false, msg: "User name incorrect or not exist or password invalid" });
    ...
    async authenticate(request, response, next) {
        try {
            ...
            if (!authorization) return response.status(401).send({ auth: false, msg: "Token not found" });

            const [, token] = authorization.split(" ");

            if (!token) return response.status(401).send({ auth: false, msg: "Token bad formatted" });
